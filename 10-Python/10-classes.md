...
Classes are the foundation of object-oriented programming (OOP) in Python. A class acts as a template (blueprint) for creating objects that share the same attributes and behaviors.
...
class Person:
    ...
    def greet(self):
        return f"Hello, my name is {self.name}."  # self is like 'this' in other languages
...
Polymorphism allows methods to use objects of different types through a common interface.
