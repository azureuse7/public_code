# Kubernetes Subscription Quota Management on AKS and EKS
...
- **Right-Sizing Node Pools:** Start with appropriately sized node pools and instance types that can handle expected workloads. Overly small nodes may cause a higher node count, quickly hitting quotas.
- **Reserve Enough IP Addresses:** For AKS, ensure sufficient IP addresses in the subnet. For EKS, ensure adequate ENIs (Elastic Network Interfaces) to prevent hitting network-related quotas early on.

### Request Increased Quotas in Advance

- **Azure:** For AKS on Azure, certain default quotas (like total cores per region) may be low. Proactively request quota increases from Azure support through the Azure Portal before your environment scales.
- **AWS:** For EKS on AWS, limits such as total EC2 instances per region or load balancer quotas may be reached. Submit service limit increase requests well before you hit production scale.

### Implement Autoscaling with Constraints

- **Horizontal Pod Autoscaling (HPA) and Cluster Autoscaler (CA):** Autoscalers can unexpectedly cause spikes in resource creation. Configure sensible maximum boundaries for the number of nodes the Cluster Autoscaler can add.
...
### Use Managed Node Groups and Spot Instances (EKS)

If you anticipate unpredictable demand, consider mixing on-demand and spot instances. This helps maintain capacity while controlling costs. Ensure you have enough allocated quota for both instance types.
...
- **Azure Monitor (AKS):** Use Azure Monitor to track usage metrics for cores, IP addresses, managed disks, and load balancers. Set alerts when usage approaches 80% of quota limits.
...
- **Cluster Autoscaler Logs:** If the autoscaler reports failed attempts to scale due to resource constraints, configure alerts on these log messages.
...
Periodically query quota usage via CLI (for example, `az vm list-usage` for Azure or `aws service-quotas list-service-quotas` for AWS) and run these checks in automated scripts. Trigger Slack or email notifications if thresholds exceed a safe margin.

## Recovery Strategies

### Immediate Scale-Down or Clean-Up

- **Free Up Resources:** If you have hit a quota limit, consider temporarily scaling down non-critical workloads, removing unused node pools, or deleting orphaned resources (such as stale load balancers or unused IP addresses).
...
### Request a Quota Increase Urgently

- **Azure:** Through the Azure Portal, raise a support request for a quota increase. This can often be expedited with the correct support plan.
...
- **Use Different Instance Families/Types:** If you cannot get a quota increase fast enough, pivot to an instance type with available capacity. For example, switch from a VM size or EC2 family that is hitting limits to a different size or family.
...
If IP exhaustion is the issue, consider using Azure CNI with a larger subnet or using Azure's IPAM solutions. Redeploying with larger subnets can immediately free you from IP address constraints.
